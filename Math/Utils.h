...
template <typename T>
static inline constexpr T lerp(const T &a, const T &b, float v)
{
	return a * (1 - v) + b * v;
}

template <typename T>
static inline constexpr T clamp(const T &value, const T &min, const T &max)
{
	if (value > max)
		return max;
	if (value < min)
...
template <typename T>
static inline constexpr const T &max(const T &v1, const T &v2)
{
	return (v1 > v2) ? v1 : v2;
}
...
template <typename T>
static inline constexpr int max_i(const T &v1, const T &v2)
{
	return (v1 > v2) ? 0 : 1;
}

template <typename T>
static inline constexpr const T &min(const T &v1, const T &v2)
{
	return (v1 < v2) ? v1 : v2;
}
