...
Mat4 Mat4_PerspectiveRH(float fov, float aspect, float znear, float zfar)
{
	Mat4 m;
...
{
	// Here we need to tranfrom from [left..right] range to [-1..1] range.
	// Let's find out linear components for that transform: scale and offset.
	//   y = Ax + B
	//
	// [-1..1] range has length of 2, so let's convert range of (right-left) length to 2
	//  1. Subtract left and divide range by its length (right-left), converts it to [0..1] range.
	//  2. Multiply by 2, converts [0..1] range to [0..2].
	//
	// Now simply convert [0..2] range to [-1..1] by applying -1 offset. The resulting formula is:
	//    y = (x - left) * 2 / (right-left) - 1
	//
	// However, let's convert it to linear form (L = left, R = right):
	//    1. Multiply x-L and 2/(R-L), gives:
	//       2x/(R-L) - 2L/(R-L)
	//    2. Extract A coefficent:
	//       2x/(R-L) = x*2/(R-L), hence A = 2/(R-L)
	//    3. Extract B coefficent:
	//       B = -2L/(R-L)+1
	//    4. We can simplify B a bit:
	//       B = -(L+L)/(R-L)+1
	//       B = -(L+L+R-L)/(R-L)
	//       B = -(L+R)/(R-L) == -(R+L)/(R-L)
	//
	...
	//
	// Let's do the same for [left..right] range to [0..1] range.
	//   y = (x - left) / (right-left)
	//   y = x/(R-L) - L/(R-L)
	//   A = 1 / (right - left)
	//   B = -left / (right-left)
